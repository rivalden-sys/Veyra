...
create table if not exists public.tenant_invitations (
  ...
  constraint tenant_invitations_no_platform_role check (role <> 'super_admin'),
  constraint tenant_invitations_email_not_blank check (length(btrim(email::text)) > 3)
);
...
create index if not exists tenant_invitations_email_idx
  on public.tenant_invitations (email, expires_at);
...
declare
  ...
  hash_value text := encode(digest(raw_token, 'sha256'), 'hex');
begin
